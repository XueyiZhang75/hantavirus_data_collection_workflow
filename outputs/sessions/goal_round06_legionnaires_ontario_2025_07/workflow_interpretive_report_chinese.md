...
- zero_case_statements: 3 条。zero-case statement 不是 confirmed case record。
- surveillance_summary_records: 4 条。surveillance summary 可能是 aggregate，不一定是 individual case record。
- outbreak_summary_records: 1 条。outbreak summary 可能是 aggregate 或 outside requested scope。
- context_records: 4 条。context/background evidence 有背景价值，但不是 case count。
- unclassified_observation_records: 19 条。unclassified observation 需要人工判断。
- non_primary_observations: 7 条。non-primary observations 保留为非主病例证据视图。
