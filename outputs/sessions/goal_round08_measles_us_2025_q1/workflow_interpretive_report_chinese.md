...
- session id: `goal_round08_measles_us_2025_q1`
- live search: `True`
- live fetch: `True`
- LLM stages: `True`
...
| final_dataset_pre_quality_gate_count | 88 |
| zero_case_statement_count | 0 |
| exposure_monitoring_record_count | 0 |
| surveillance_summary_record_count | 2 |
| outbreak_summary_record_count | 6 |
| context_record_count | 6 |
| unclassified_observation_count | 6 |
| non_primary_observation_count | 13 |
| quarantined_record_count | 15 |
| pending_review_record_count | 73 |
...
- source verification chain: predicted=1, discovered=1, fetched=1, fetch_failed=0, parsed=1, unusable=1, chunks=874, records=88, extracted=1, accepted=0
- failure_stage: `records_quarantined`.
- official_extraction_failure_reasons: `{'must_fetch_source_not_attempted_for_extraction': 16, 'must_fetch_source_partially_skipped_due_to_chunk_cap': 2}`
...
- surveillance_summary_records: 2 条。surveillance summary 可能是 aggregate，不一定是 individual case record。
- outbreak_summary_records: 6 条。outbreak summary 可能是 aggregate 或 outside requested scope。
- context_records: 6 条。context/background evidence 有背景价值，但不是 case count。
- unclassified_observation_records: 6 条。unclassified observation 需要人工判断。
- non_primary_observations: 13 条。non-primary observations 保留为非主病例证据视图。

## 6. 跨来源印证结果

- claim_count: `97`
- claim_comparison_count: `4656`
- corroborated_event_count: `14`
- corroborated_primary_case_event_count: `0`
- conflicting_claim_count: `48`
- single_source_unverified_count: `10`
...
- source_identity_assessed_count: `34`
- actual_publisher_unknown_count: `14`
- source_type_counts: `{'official_public_health_agency': 7, 'national_public_health_agency': 6, 'unknown': 3, 'secondary_aggregator': 1, 'news_media': 6, 'state_or_local_public_health_agency': 1, 'academic_or_peer_reviewed_source': 3, 'structured_database': 1, 'background_fact_sheet': 1, 'international_public_health_agency': 5}`
...
- human_review_item_count: `119`
- 优先检查：是否有 source scope mismatch、validation limitation、publisher uncertainty、single-source unverified claims。
- review_source_credibility_src_search_3c843cdae62c: missing_publisher
- review_source_src_search_3c843cdae62c: Source classified as data_source; both screening and critic agree to include for content fetch.
- review_source_credibility_src_search_0132cfa4f779: missing_publisher
- review_source_src_search_0132cfa4f779: Source classified as data_source; both screening and critic agree to include for content fetch.
