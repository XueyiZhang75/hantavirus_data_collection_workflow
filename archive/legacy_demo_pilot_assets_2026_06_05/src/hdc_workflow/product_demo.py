"""Shared runtime configuration helpers for HDC workflow runs.

This module keeps backward-compatible product-demo names, but the primary
concept is a workflow runtime profile: one config controls the graph input,
source overlays, live fetch, LLM stages, source roles, and output paths.
"""
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "workflow_runs" / "new_mexico_hps_live_llm"
DEFAULT_WORKFLOW_RUN_CONFIG_PATH = PROJECT_ROOT / "configs" / "hdc_workflow_run_config.jsonc"
# Backward-compatible alias. New code should use DEFAULT_WORKFLOW_RUN_CONFIG_PATH.
DEFAULT_PRODUCT_CONFIG_PATH = DEFAULT_WORKFLOW_RUN_CONFIG_PATH
...
def default_product_demo_config() -> dict:
    ...
    return {
        ...
        "output": {
            "run_output_dir": str(DEFAULT_OUTPUT_DIR.relative_to(PROJECT_ROOT)),
        },
    }
...
def resolve_product_config_path(config_path: str | Path | None = None) -> Path:
    """Resolve a workflow runtime config path relative to the project root."""

    path = Path(config_path) if config_path else DEFAULT_PRODUCT_CONFIG_PATH
    if not path.is_absolute():
        path = PROJECT_ROOT / path
    return path


def load_product_demo_config(config_path: str | Path | None = None) -> dict:
    """Load workflow runtime settings from JSON, with stable built-in defaults."""

    path = resolve_product_config_path(config_path)
    config = default_product_demo_config()
    if path.exists():
...
def product_demo_env_from_config(config: dict) -> dict[str, str]:
...
def product_demo_config_with_overrides(
    config: dict,
    *,
...
def studio_initial_state_from_config(
    config: dict,
    *,
...
def product_demo_output_dir_from_config(config: dict) -> Path:
    """Resolve the run output directory from the centralized config."""

    output = config.get("output") or {}
    output_dir = Path(output.get("run_output_dir") or DEFAULT_OUTPUT_DIR)
...
def load_workflow_run_config(config_path: str | Path | None = None) -> dict:
    ...
    return load_product_demo_config(resolve_workflow_run_config_path(config_path))


def workflow_run_env_from_config(config: dict) -> dict[str, str]:
    """Build environment variables for the configured workflow run."""

    return product_demo_env_from_config(config)


def workflow_run_config_with_overrides(config: dict, **kwargs) -> dict:
    """Return a workflow runtime config copy with one-run overrides applied."""

    return product_demo_config_with_overrides(config, **kwargs)
...
def workflow_initial_state_from_config(
    ...
    *,
    ...
    return studio_initial_state_from_config(
        config,
        include_empty_fields=include_empty_fields,
    )


def workflow_output_dir_from_config(config: dict) -> Path:
    """Resolve the run output directory from a runtime profile."""

    return product_demo_output_dir_from_config(config)
...
@contextmanager
def temporary_product_env(updates: dict[str, str]):
